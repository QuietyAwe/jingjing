...
src/components/     # 通用 UI 组件（ChatBubble、InputBar、CollapsibleSection）
...
| v1.7 | 07-19 | 修复 | 巩固流事件列表包含默认事件：LLM 可将片段挂靠到"日常闲聊" | 默认事件修复 |
| v1.7 | 07-19 | 功能 | 删除本周时间表：设置页数据管理区新增删除按钮 | 时间表管理 |
| v1.7 | 07-19 | UI | 设置页折叠：8 个区域均可折叠/展开，减少滚动 | 设置页优化 |
| v1.7 | 07-19 | 功能 | AI 提取人设：用户输入一段话，AI 自动提取为 UserInfo 结构化画像 | 人设编辑 |
| v1.7 | 07-19 | UI | 默认事件可见："日常闲聊"在设置页事件列表中显示 | 事件管理 |
...
| **P11-1** | 修复 | 巩固流事件列表包含默认事件，LLM 可正常挂靠片段到"日常闲聊" | 已完成 |
| **P11-2** | 功能 | 删除本周时间表：设置页数据管理区新增删除按钮 | 已完成 |
| **P11-3** | UI | 设置页折叠：CollapsibleSection 组件，8 个区域可折叠/展开 | 已完成 |
| **P11-4** | 功能 | AI 提取人设：用户输入自由文本 → LLM 提取为 UserInfo → merge 合并 | 已完成 |
| **P11-5** | UI | 默认事件"日常闲聊"在设置页事件列表中可见 | 已完成 |
